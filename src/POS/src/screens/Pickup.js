...
const Pickup = () => {
  ...
  const { loc_id } = useSelector((state) => state.root.main);
  useEffect(() => {
    let params = {
      loc_id: loc_id,

    };
    APIHandler.hitApi(PICKUP_ORDERS, 'POST', params).then(response => {
      console.log('pickup ka response', response);
      setPickup(response);
    });
  }, []);
  ...
  const pickupOrder = () => {
    let params = {
      order_id: pickup.map(i => i.id)
    }
    APIHandler.hitApi(PICKUP_DETAILS, 'POST', params).then(response => {
      console.log('pickup DETAILS ka response', response);
      setPickupDetails(response);
    });
  }
  ...
  return (
    ...
    <>
      {state === 'Subpickup' ? <Subpickup pickupDetails={pickupDetails} /> :
        <View>
              ...
              renderItem={({ item }) => {
                return (
                  <View style={{ flexDirection: 'row', backgroundColor: item.bcolor }}>
                    ...
                    <View style={styles.viewStyle}>
                      <Text style={[styles.textStyle, { fontWeight: 'bold' }]}>{item.id}</Text>
                    </View>
                    <View style={styles.viewStyle}>
                      <Text style={[styles.textStyle, { color: '#FF2E2E' }]}>{item.Time}</Text>
                    </View>
                    <View style={styles.viewStyle}>
                      <Text style={[styles.textStyle, { fontWeight: 'bold' }]}>{item.user}</Text>
                    </View>
                    <View style={styles.viewStyle}>
                      <Text style={[styles.textStyle, { color: '#FF2E2E' }]}>{item.order_status}</Text>
                    </View>
