...
var _listeners = [];
var dateFormat = moment(new Date());
const base64Image =
...
const SubTakeway = props => {
  ...
  const {select, c_Screen, stf_id, loc_id, printer_address} = useSelector(
  ...
  const [table_order_detail, setTable_order_detail] = useState({
    arr: [
      {
        id: 503,
        mem: 1,
    ...
    arry2: [
      {id: 402, name: 'pizza 2 Large', quantity: 1, unit_price: '800.00'},
    ],
  });
  ...
  const addNewItem = (name, price, Dis, id, Var, a) => {
    ...
    if (check !== -1) {
      ...
      if (array[check].dis_sign == '$') {
        array[check].p =
          parseInt(array[check].p) +
          parseInt(array[check].price) -
          parseInt(array[check].dis);
      } else if (array[check].dis_sign == '%') {
        array[check].p =
          parseFloat(array[check].p) +
          parseFloat(array[check].price) -
          (parseFloat(array[check].dis) / 100) * parseFloat(array[check].price);
        // console.log("Discount  is =", ((parseFloat(array[check].dis) / 100) * parseInt(array[check].price)))
      } else {
        array[check].p =
          parseInt(array[check].p) + parseInt(array[check].price);
      }
  ...
  useEffect(() => {
    ...
    if (mounted) {
      ...
      APIHandler.hitApi(Print_Slip, 'POST', param).then(response =>
        setSlip(response),
      );
      setLoad(true);
      let params = {
  ...
  const item_Dis = () => {
    if (isSwitchOn == false) {
      ...
      var RowPrice =
        parseInt(localArr[selectedIndex].price) * localQuantity - localDiscount;

      if (
        parseInt(localDiscount) >
        parseInt(localArr[selectedIndex].p - modifier_Sum)
      ) {
        ToastAndroid.show(
          'Discount is Greater then item price',
          ToastAndroid.SHORT,
        );
      ...
      var RowPrice =
        parseInt(localArr[selectedIndex].price) * localQuantity -
        (localDiscount / 100) *
          parseInt(localArr[selectedIndex].price) *
          localQuantity +
        modifier_Sum;
      if (RowPrice > parseInt(localArr[selectedIndex].p) * localQuantity) {
        ...
        localDis_sign = '%';
        total_discount =
          total_discount + parseFloat((RowPrice * localDiscount) / 100);
  ...
  const Save_Order = () => {
    ...
    APIHandler.hitApi(Takeaway_Order, 'POST', param).then(response =>
      setOrder(response),
    );
  };
  ...
  const Table_Booked_but_not_Paid = () => {
    ...
    APIHandler.hitApi(Table_res, 'POST', param).then(response =>
      console.log(' Table_Booked_but_not_Paid ', response),
    );
    setState(props.pass);
  };
  ...
  const Table_Free = () => {
    ...
    if (modal == false) {
      ...
      APIHandler.hitApi(Table_Close, 'POST', param).then(response =>
        console.log(' Table_Free_====== ', response),
      );
    }
  };
  ...
  const item_qty = i => {
    ...
    if (localsing == '$') {
      var rowPrice =
        parseInt(localArr[i].p) * localQuantity - localD + modifier_Sum;
      setP(p + rowPrice);
      dispatch(SetTotal2(p + rowPrice));
    } else if (localsing == '%') {
      var rowPrice =
        parseInt(localArr[i].p) * localQuantity -
        (localD / 100) * parseInt(localArr[i].p) * localQuantity +
        modifier_Sum;
      setP(p + rowPrice);
      dispatch(SetTotal2(p + rowPrice));
    } else {
    ...
    return (
      <>
        ...
        <View
          style={{
            flexDirection: 'row',
            backgroundColor: item.key % 2 == 0 ? 'white' : '#F7F7F7',
          }}>
          <View
            style={{
              borderRightWidth: 1,
          ...
          <TouchableOpacity
            disabled={true}
            style={{justifyContent: 'center', width: wp('22.9%')}}
            onPress={() => {
          ...
          <TouchableOpacity
            style={[
              styles.viewStyle,
              {alignItems: 'center', width: wp('6.7%'), height: hp('10%')},
            ]}
            onPress={() => {
          ...
          <View
            style={[styles.viewStyle, {width: wp('7.1%'), height: hp('10%')}]}>
            <Text
              style={[
                styles.textStyle,
                {alignSelf: 'center', color: '#FC3F3F'},
              ]}>
              ${item.p}
            </Text>
          </View>
          <TouchableOpacity
            style={[
              styles.viewStyle,
              {alignItems: 'center', width: wp('5%'), height: hp('10%')},
            ]}
            onPress={() => {
    ...
    return (
      <View
        ...
        <View
          style={[styles.viewStyle, {width: wp('7.1%'), height: hp('10%')}]}>
          <Text
            style={[styles.textStyle, {alignSelf: 'center', color: '#FC3F3F'}]}>
            ${item.p}
          </Text>
        </View>
        <TouchableOpacity
        ...
        <View
          ...
          <View style={{width: '30%', height: '30%', backgroundColor: 'white'}}>
            <View
              ...
              <Text style={{fontWeight: 'bold', fontSize: wp('1.4%')}}>
                Variant Name
              </Text>
              <Text style={{fontWeight: 'bold', fontSize: wp('1.2%')}}>
                Price
              </Text>
            </View>
            <FlatList
              ...
              renderItem={({item, index}) => {
                return (
                  <TouchableOpacity
                    ...
                    <Text style={{fontSize: wp('1%'), color: 'red'}}>
                      {'$' + item.price}
                    </Text>
                  </TouchableOpacity>
                );
  ...
  return (
    <>
      {state == 'Profile' ? (
        ...
        <Takeway
          T_order_id={props.T_order_id}
          mergedTables={props.mergedTables}
          br={props.br}
          Table={props.Table}
        ...
        <>
            ...
            <View
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  onPress={() => increment()}>
                  <Image
                    source={require('../../assets/plus_c.jpg')}
                    resizeMode="contain"
                    style={{
                      width: wp('10%'),
                      height: hp('10%'),
          ...
          <Modal
            ...
            <View style={[styles.modalView, {width: '20%', height: '50%'}]}>
              ...
              <View
                ...
                <Image
                  source={require('../../assets/warning-removebg-preview.png')}
                  resizeMode="contain"
                  style={{width: '100%', height: '100%'}}
                />
              </View>
              <Text style={{fontSize: wp('1.4%'), color: 'black'}}>
                Close sale for this table and{' '}
                <Text style={{fontWeight: 'bold', color: 'black'}}>
                  reopen for new customer
                </Text>
              </Text>
          ...
          <Modal
            ...
            <View style={[styles.modalView, {width: '35%'}]}>
              <TouchableOpacity
                style={{alignSelf: 'flex-end', marginRight: 10, marginTop: 0}}
                onPress={() => setShow_m(false)}>
                <Image
                  source={require('../../assets/cross.jpg')}
                  resizeMode="contain"
                  style={{width: 20, height: 20}}
              ...
              <View
                ...
                <Image
                  source={require('../../assets/conf.png')}
                  resizeMode="contain"
                  style={{width: '100%', height: '100%'}}
                />
              </View>
              <Text style={{fontSize: wp('1.4%'), marginTop: '5%'}}>
                Will you discard the order?
              </Text>
              <View
                style={{
                  flex: 1,
          ...
          <Modal
            ...
            <View
              style={[styles.modalView, {width: wp('35%'), height: hp('55%')}]}>
              <TouchableOpacity
                style={{
                  marginBottom: hp('1%'),
                  alignSelf: 'flex-end',
              ...
              <Text style={{fontSize: wp('1.4%'), marginTop: hp('5%')}}>
                Will you discard the order?
              </Text>
              <View
                style={{
          ...
          {state === 'sub' ? (
                  ...
                  <>
                      ...
                      <View
                        ...
                        <Text
                          ...
                          {props.table_pass == 'table'
                            ? table_order_detail.arr.map(i => i.table)
                            : tableNames}
                        </Text>
                        <View
                          style={{
                        ...
                        <TouchableOpacity
                          ...
                          <Text
                            ...
                            {cBtn == 1
                              ...
                              ? props.Count
                              : props.table_pass == 'table'
                              ? table_order_detail.arr.map(i => i.mem)
                              : null}
                            PAX
                          </Text>
                        </TouchableOpacity>
                    ...
                    <View
                      ...
                      <View
                        style={[
                          styles.viewStyle,
                          {alignItems: 'flex-start', width: wp('24%')},
                        ]}>
                        <Text
                          style={[
                            styles.textStyle,
                          ...
                          <View>
                            ...
                            <TouchableOpacity
                              ...
                              <Image
                                ...
                                source={require('../../assets/plus_cc.png')}
                              />
                              <Text
                                style={{
                                  color: 'black',
                              ...
                              <View
                                ...
                                {i.address_line_1 == null ? (
                                  ...
                                  <Text
                                    ...
                                    {contactDetails.map(i => i.address_line_1)}
                                  </Text>
                                )}
                                {i.address_line_2 == null ? (
                                  ...
                                  <Text
                                    ...
                                    {contactDetails.map(i => i.address_line_2)}
                                  </Text>
                                )}
                              </View>
                            </View>
                      ...
                      <View
                        ...
                        <View
                          ...
                          <View style={styles.vStyle}>
                            <Text style={[styles.Tstyle, {flex: 1}]}>
                              Sub Total:
                            </Text>
                          ...
                          <View style={styles.vStyle}>
                            ...
                            <Text style={styles.Tstyle}>
                              ...
                              {totDis == 0
                                ? 0
                                : parseFloat(sub_Sum - totDis).toFixed(2)}
                            </Text>
                          </View>
                        </View>
                        <View
                          ...
                          <Text
                            ...
                            ${disTot == 1 ? totDis : parseFloat(sub_Sum)}.00
                          </Text>
                        </View>
                      </View>
                    </View>
              ...
              <View style={{flex: 0.4, backgroundColor: 'white'}}>
                {select == 'reservation' ? (
                  ...
                  <Pay
                    NameState={nameState}
                    disTotal={CBDisTotal}
                    btnDisTot={CBTotalDis}
                    refresh={reload}
                    pay={
                      props.table_pass == 'table'
                        ? table_order_detail.arr.map(i => i.total)
                        : props.table_pass == 'notable'
                        ? sub_Sum
                        : null
                    }
                    ...
                    userid={props.userId}
                    t_id={tableIds}
                    member={cBtn == 0 ? props.Count : paxUpd}
                    reload={reload}
                    empty={Empty}
                    Call={Callback}
                    function={fun}
                    statename={props.pass}
                    Cat_id={cat_id}
                    T_order_id={
                      props.table_pass == 'table'
                        ? table_order_detail.arr.map(i => i.id)
                        : null
                    }
                    table_pass={props.table_pass}
                    T_order_sum={
                  ...
                  <>
                    ...
                    <View
                      ...
                      {select == 'dis' ? (
                        ...
                        <TextInput
                          placeholder=" "
                          style={{fontSize: wp('4%'), marginLeft: wp('3%')}}>
                          {phone}
                        </TextInput>
                      ) : null}
                    </View>

                    <View style={{width: wp('38%'), height: hp('58.5%')}}>
                      <View
                        style={{
                          flexDirection: 'row',
                          width: wp('39%'),
                  ...
                  <View style={{flex: 1}}>
                    ...
                    <View
                      ...
                      <Text style={{fontSize: wp('1.5%'), color: 'gray'}}>
                        Comment
                      </Text>
                    </View>
            ...
            <MainDashborad br={br} Key={Id} />
          ) : state == 'blue' ? (
            <App br={br} Key={Id} Empty={Empty} />
          ) : null}
