...
const Login = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    APIHandler.hitApi(Branches, 'POST', params).then(response => {
      setItem(response);
  ...
  return (
    <View style={styles.container}>
      <View
        style={{marginTop: '3%', marginLeft: 10, justifyContent: 'flex-start'}}>
        <Image
                ...
                {item.location_id == branchid ? (
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      console.log('STF_ID is =', item.id);
                    }}>
                    <View style={styles.UserView}>
                      <Image
                        source={require('../assets/profile.jpg')}
                        resizeMode="contain"
                        style={{width: '80%', height: '80%', borderRadius: 240}}
                      />
                    </View>
